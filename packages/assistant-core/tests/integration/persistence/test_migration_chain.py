...
_NEW_TASK = text(
    ...
    " (id, conversation_id, user_id, tool_name, status)"
    " VALUES (:id, :conversation_id, :user_id, 'run_a_long_thing', 'pending')"
)
_NEW_PROGRESS = text(
    "INSERT INTO task_progress (task_id, percent) VALUES (:task_id, 0.25)"
)
# A host's own task tables, which name no table this chain creates later.
_HOST_TASK_TABLES = (
    (
        ...
        " phase_overrides JSONB NOT NULL DEFAULT '{}',"
        " result JSONB,"
        " error TEXT,"
...
async def test_the_chain_refuses_a_database_that_holds_one_task_table(
    db_engine: AsyncEngine,
) -> None:
    """The task pair is refused half-built, the way the others are."""
    engine = await _fresh_database(db_engine, "assistant_core_half_tasks")
    try:
        async with engine.begin() as connection:
            await connection.run_sync(Base.metadata.create_all)
            await connection.exec_driver_sql("DROP TABLE task_progress")
            with pytest.raises(RuntimeError) as caught:
                await connection.run_sync(_migrate)

        assert "present ['background_tasks']" in str(caught.value)
        assert "missing ['task_progress']" in str(caught.value)
    finally:
        await engine.dispose()


async def test_the_chain_keys_a_task_tagged_chunk_to_the_task_table(
    chain_engine: AsyncEngine,
) -> None:
    """The baseline leaves this key to the revision that creates the table."""
    async with chain_engine.connect() as connection:
        keys = await connection.run_sync(_foreign_keys, "conversation_events")

    assert keys == {
        ("conversation_id", "conversations", "id"),
        ("task_id", "background_tasks", "id"),
    }


async def test_a_task_and_its_progress_go_when_the_thread_does(
    chain_engine: AsyncEngine,
) -> None:
    """The cascade the chain wrote leaves no task row behind a deleted thread."""
    ...
    async with maker() as session:
        ...
        await session.execute(_NEW_PROGRESS, {"task_id": str(task_id)})
        await session.commit()
        await session.execute(
            text("DELETE FROM conversations WHERE id = :id"),
            {"id": str(conversation_id)},
        ...
        tasks = await session.execute(select(BackgroundTask))
        progress = await session.execute(select(TaskProgressRow))

    assert list(tasks.scalars()) == []
    assert list(progress.scalars()) == []


async def test_a_task_row_needs_a_user_the_host_supplies(
    chain_engine: AsyncEngine,
) -> None:
    """`users` is the one host table a durable task still reaches."""
...
async def test_a_task_written_on_the_chain_schema_reads_back(
    chain_engine: AsyncEngine,
) -> None:
    """A row that names only the required columns takes the chain's defaults."""
    user_id, conversation_id, task_id = uuid4(), uuid4(), uuid4()
    maker = async_sessionmaker(
        chain_engine, class_=AsyncSession, expire_on_commit=False
    )
    async with maker() as session:
        ...
        task = (await session.execute(select(BackgroundTask))).scalar_one()
        progress = (await session.execute(select(TaskProgressRow))).scalar_one()

    assert task.args == {}
    assert task.phase_overrides == {}
    ...
    assert progress.emitted_at is not None


async def test_the_key_is_added_when_a_host_chain_built_the_task_tables(
    db_engine: AsyncEngine,
) -> None:
    """The chain builds the chunk table, so it writes the key its own baseline left."""
    engine = await _fresh_database(db_engine, "assistant_core_host_tasks")
    try:
        async with engine.begin() as connection:
            await connection.run_sync(Base.metadata.create_all, tables=HOST_TABLES)
            for statement in _HOST_TASK_TABLES:
...
async def test_the_key_is_not_added_twice_when_a_host_chain_built_it(
    db_engine: AsyncEngine,
) -> None:
    """A database whose own chain wrote the key keeps exactly one."""
    engine = await _fresh_database(db_engine, "assistant_core_own_key")
    try:
        async with engine.begin() as connection:
            await connection.run_sync(Base.metadata.create_all)
            await connection.run_sync(_migrate)
        async with engine.connect() as connection:
            named = await connection.run_sync(
                lambda sync: [
                    key["name"]
                    for key in inspect(sync).get_foreign_keys("conversation_events")
