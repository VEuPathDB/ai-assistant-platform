...
`--mcp-endpoint`, `--mcp-bearer`, `--mcp-bearer-second` and
`--mcp-bearer-minimum` also read `MCP_CONFORMANCE_ENDPOINT`,
`MCP_CONFORMANCE_BEARER`, `MCP_CONFORMANCE_BEARER_SECOND` and
`MCP_CONFORMANCE_BEARER_MINIMUM`, so a credential never has to appear on a
command line.
