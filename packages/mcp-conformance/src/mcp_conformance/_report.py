...
# itself, so a shortened repr cannot carry one. It is half the suite's own
# default minimum, so a run that long is the credential and not prose, and a
# target that relaxes its minimum does not lower this floor.
