...
from mcp_conformance._evidence import ToolRecord
from mcp_conformance._options import (
    BEARER_MINIMUM_ENV,
    BEARER_MINIMUM_OPTION,
    DEFAULT_BEARER_MINIMUM,
    DEFAULT_META_NAMESPACE,
    META_NAMESPACE_ENV,
    META_NAMESPACE_OPTION,
    ConformanceTarget,
)
...
OWN_NAMESPACE = "com.example.assistant"

DECLARING_TOOL = {
    "name": "read_thing",
    "inputSchema": {"type": "object"},
    "_meta": {
        f"{OWN_NAMESPACE}/streamPart": {"kind": "data-thing", "version": 1},
        f"{OWN_NAMESPACE}/maxCallSeconds": 90,
    },
}


def test_a_target_that_names_no_namespace_reads_the_organisation_default() -> None:
    assert ConformanceTarget(endpoint=ENDPOINT).meta_namespace == DEFAULT_META_NAMESPACE


def test_the_runner_names_the_namespace_on_the_command_line() -> None:
    config = _Config({"--mcp-endpoint": ENDPOINT, META_NAMESPACE_OPTION: OWN_NAMESPACE})

    target = target_of(config)

    assert target is not None
    assert target.meta_namespace == OWN_NAMESPACE


def test_the_namespace_falls_back_to_the_environment(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv(META_NAMESPACE_ENV, OWN_NAMESPACE)
    config = _Config({"--mcp-endpoint": ENDPOINT})
...
def test_a_tool_annotated_for_the_named_namespace_declares_its_part() -> None:
    """A server annotated for its own deployment is read, not reported as silent."""
    tool = ToolRecord.model_validate(DECLARING_TOOL)

    declared = tool.stream_part(OWN_NAMESPACE)

    assert declared is not None
    assert declared.kind == "data-thing"
    assert tool.declared_max_call_seconds(OWN_NAMESPACE) == 90.0


def test_a_tool_annotated_for_another_namespace_declares_nothing() -> None:
    tool = ToolRecord.model_validate(DECLARING_TOOL)

    assert tool.stream_part(DEFAULT_META_NAMESPACE) is None
    assert tool.declared_max_call_seconds(DEFAULT_META_NAMESPACE) is None
