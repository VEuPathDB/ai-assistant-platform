...
`--mcp-bearer-minimum`, `--mcp-report`, `--mcp-sample-args`,
`--mcp-slow-tool`, `--mcp-isolation-tool`, `--mcp-max-call-seconds`). The two
credentials, the endpoint and the minimum also read environment variables,
because a credential on a command line is a credential in a shell history. The
shortest bearer a target admits is the operator's policy, not this suite's: the
default is 32 and a deployment whose registry issues another length states it.
...
**The families as a test module in the host application.** It is the smallest change and
