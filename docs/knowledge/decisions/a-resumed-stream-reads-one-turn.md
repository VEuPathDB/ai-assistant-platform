...
description: The AI SDK builds one message per stream and rebuilds a resumed one from empty, so DurableChatTransport ends a resumed stream at a `start` that opens another turn, replays the open message whole, and serves the rest to the next resume; merging the continuation into the suspending message, a second HTTP tail, and giving `data-background-task-started` an `id` were all rejected.
...
The SDK reads one assistant message per stream. `AbstractChat.makeRequest`
seeds a resumed read with no message, so that read rebuilds the message it
names from empty and the reader's `start` case only renames what it builds. Two
rules follow: a tail that carries a suspended turn's gap and then the
continuation turn ends with the continuation holding every part of the turn
before it, and a resumed tail that does not replay the open message whole
rebuilds it without the parts the client already had.
...
that exists to receive it, and it keeps section 1's rule that the thread holds
each id once, because the SDK appends a rebuilt message the thread does not
hold last. PROTOCOL 1.5.2 states the reader rule in section 9.
